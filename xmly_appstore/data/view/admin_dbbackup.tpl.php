...
</table><? } ?><form action="index.php?admin_db/backup" method="post" onsubmit="return docheck();">
...
<input type="radio" name="type" value="full" class="radio" checked="checked" onclick="document.getElementById('showtables').style.display='none'">全部备份(推荐)，备份Tipask数据库所有表<br />
...
<tr>
<? if(is_array($tables)) { foreach($tables as $key => $value) { if($key%4!=0) { ?><td class="altbg2"><input type="checkbox" class="checkbox" value="<?=$value?>" name="tables[]"/><?=$value?></td><? } else { ?></tr>
<tr>
